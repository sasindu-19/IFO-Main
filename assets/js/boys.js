...
function openProduct(id) {
  ...
  document.title = `${p.name} — Boys' Fashion | IFO Galle`;
  const metaDesc = document.querySelector('meta[name="description"]');
  if (metaDesc) {
    metaDesc.setAttribute("content", `${p.name} - ${p.description || "Premium boys' clothing item from Ikram Fashion Outlet Galle."} Price: LKR ${p.price.toLocaleString()}.`);
  }
